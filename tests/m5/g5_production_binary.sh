...
default_go=go
if [ -x /path/to/naive-m4/go1.25.12/bin/go ]; then
...
case "$platform" in
  MINGW*|MSYS*|CYGWIN*)
    ...
    # filesystem path before openssl.exe sees it.
    if [ -n "${MSYS2_ARG_CONV_EXCL:-}" ]; then
      MSYS2_ARG_CONV_EXCL="$MSYS2_ARG_CONV_EXCL;/CN="
    else
...
expected_forwardproxy=${M5_EXPECTED_FORWARDPROXY:-964281a9797efd9a4c953f6273c73e397e777864}
