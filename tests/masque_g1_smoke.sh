...
server_port="${MASQUE_SERVER_PORT:-19661}"
echo_port="${MASQUE_ECHO_PORT:-19001}"
authority="[::1]:${server_port}"
uri_template="https://${authority}/.well-known/masque/udp/{target_host}/{target_port}/"
test_dir="$(mktemp -d "${TMPDIR:-/tmp}/naive-masque-g1.XXXXXX")"
server_pid=""
echo_pid=""
